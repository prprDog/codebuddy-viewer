...
- **置顶开关**：默认不置顶（避免遮挡弹出登录窗口）；点击一键切换置顶状态，图标随之变化（📌=未置顶，📍 高亮=置顶中）。
- **积分实时展示**：调用 CodeBuddy 官方接口获取本月积分使用明细、每日用量、积分余额。
- **积分套餐总览（进度条）**：调用 `get-user-resource` 汇总用户全部积分套餐，以进度条形式展示「已用 / 总量 / 剩余」与使用百分比。套餐数据为低频变化且接口存在限流，采用 **5 分钟缓存 + 失败回退**策略：自动刷新复用缓存，接口异常时保留上次成功数据，进度条不因偶发限流而消失。
...
2. 登录成功后检测到导航到 `/profile/*`，自动捕获 `.codebuddy.cn` 域下的全部 Cookie 并写入本地会话文件（`userData/codebuddy-session.json`），同时同步写回主进程的持久分区，保证重启后分区 Cookie 完整。
3. 主进程 API 请求**双通道**：优先使用登录会话分区的 `fetch`（自动携带匹配 Cookie，行为与浏览器一致）；若分区返回 `401` 或网络异常，自动回退到 Node `https` 携带会话文件 Cookie 请求，两路都通过鉴权。
...
- **登录态持久化**：会话 Cookie 保存在 Electron 用户数据目录（`%APPDATA%/codebuddy-credit-widget/codebuddy-session.json`），登录时同步写回持久分区。Cookie 长期有效，无需每次启动都扫码；API 请求会自动在「分区 fetch → 会话文件回退」之间切换，避免重启后误判登录失效。
